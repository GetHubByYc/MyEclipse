...
public class Swing_Tree extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(btn_pre)) {
			enum_tree = root.preorderEnumeration();
		}else if(e.getSource().equals(btn_pst)){
			enum_tree = root.postorderEnumeration();
		}else if(e.getSource().equals(btn_brh)){
			enum_tree = root.breadthFirstEnumeration();
		}else if(e.getSource().equals(btn_dep)){
			enum_tree = root.depthFirstEnumeration();
		}
		
		while(enum_tree.hasMoreElements()){
			DefaultMutableTreeNode node = (DefaultMutableTreeNode) enum_tree.nextElement();
